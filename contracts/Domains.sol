...
contract Domains is ERC721URIStorage {
    ...
    constructor(string memory _tld) ERC721('Bunny Name Service', 'BNS') payable {
        owner = payable(msg.sender);
        tld = _tld;
        console.log('%s name service deployd', _tld);
    }
    ...
    function register(string calldata name) public payable{
        ...
        console.log('Registering %.% on the contract wiht tokenID %d', name, tld, newRecordId);

        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name": "',
                _name,
                '", "description": "A domain on the bunny name service", "image": "data:image/svg+xml;base64,',
    ...
    function getAllNames() public view returns (string[] memory) {
        console.log('Grabbing all registerd names');
